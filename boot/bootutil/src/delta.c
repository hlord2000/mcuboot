...
#define BOOT_DELTA_VERSION_RESTORE 2
#define BOOT_DELTA_HEADER_SIZE 24
#define BOOT_DELTA_HEADER_RESTORE_SIZE 32
                       ...
                       uint32_t *old_data_off, uint32_t *next_off)
{
    ...
    *new_data_off = off + sizeof(*rec);
    data_size = rec->size;
    if (boot_delta_has_restore(delta)) {
        if (data_size > (UINT32_MAX - rec->size)) {
            return -1;
        }
        data_size += rec->size;
...
static int
boot_delta_read_header_flags(const struct image_header *patch_hdr,
                             const struct flash_area *fap_secondary,
                             struct boot_delta_header *delta)
{
    if (delta->version != BOOT_DELTA_VERSION_RESTORE) {
        return 0;
    }

    return LOAD_IMAGE_DATA(patch_hdr, fap_secondary,
                           patch_hdr->ih_hdr_size + BOOT_DELTA_HEADER_SIZE,
                           &delta->flags,
                           BOOT_DELTA_HEADER_RESTORE_SIZE - BOOT_DELTA_HEADER_SIZE);
}
                           ...
                           const struct boot_delta_header *delta)
{
    ...
    if (delta->version == BOOT_DELTA_VERSION) {
        return delta->header_size == BOOT_DELTA_HEADER_SIZE ? 0 : BOOT_EBADIMAGE;
    }

    if (delta->version == BOOT_DELTA_VERSION_RESTORE) {
        if (delta->header_size != BOOT_DELTA_HEADER_RESTORE_SIZE ||
            delta->header_size > patch_hdr->ih_img_size ||
            (boot_delta_has_restore(delta) &&
             (delta->base_size == 0 || delta->write_size < delta->base_size))) {
            return BOOT_EBADIMAGE;
        }
        return 0;
    }
...
#ifdef MCUBOOT_DELTA_DFU_RESTORE
static bool
...
boot_delta_apply(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    if (delta.version == BOOT_DELTA_VERSION_RESTORE &&
        patch_hdr->ih_img_size < BOOT_DELTA_HEADER_RESTORE_SIZE) {
        BOOT_LOG_ERR("Delta payload too small for restore header");
        return BOOT_EBADIMAGE;
    }

    rc = boot_delta_read_header_flags(patch_hdr, fap_secondary, &delta);
    if (rc != 0) {
        return BOOT_EFLASH;
    }
    ...
    if (restore && !boot_delta_has_restore(&delta)) {
        BOOT_LOG_ERR("Delta restore requested for non-revertible image");
        return BOOT_EBADIMAGE;
    }
    stage_restore = boot_delta_should_stage_restore(&delta, bs);
#else
    if (restore) {
        BOOT_LOG_ERR("Delta restore requested without restore support");
        return BOOT_EBADIMAGE;
    }
#endif
    ...
    if (memcmp(actual_primary_hash, expected_start_hash, IMAGE_HASH_SIZE) != 0) {
        BOOT_LOG_ERR("Delta starting image hash mismatch");
        return BOOT_EBADIMAGE;
    }
    ...
    if (rc != 0) {
        image_size = restore && delta.base_size != 0 ? delta.base_size : delta.target_size;
    }
