...
primary slot and adds a header and trailer that the bootloader is expecting:
    ...
    Options:
      ...
      --delta-base filename           Create a signed delta image against an
                                      already-signed base image instead of a full
                                      update image.
      --delta-block-size INTEGER      Granularity, in bytes, used when comparing
                                      base and target images for --delta-base.
      --delta-revertible              Include old bytes in the signed delta image
                                      so a test update can be restored from the
                                      same secondary-slot image.
...
The `--delta-base` option creates a signed delta update image instead of a full
target image. The base input must be the signed image currently running from the
primary slot. The target input is signed in memory, compared against the signed
base image, and emitted as a signed delta image containing only changed blocks.
The optional `--delta-revertible` flag stores the previous contents of each
changed block next to the new contents. This makes the delta image larger, but
allows a restore-enabled bootloader to revert an unconfirmed test update without
keeping a full backup image in the secondary slot.
