...
DELTA_VERSION = 1
DELTA_VERSION_RESTORE = 2
DELTA_FLAG_RESTORE = 0x00000001
DELTA_HEADER_SIZE = 24
DELTA_HEADER_RESTORE_SIZE = 32
DELTA_HEADER = '<IHHIIII'
DELTA_HEADER_RESTORE = '<IHHIIIIII'
DELTA_RECORD = '<II'
...
def build_delta(base_image, target_image, block_size, erased_val, hash_algorithm,
    ...
    if reversible:
        payload = bytearray(struct.pack(
            DELTA_HEADER_RESTORE,
            DELTA_MAGIC,
            DELTA_VERSION_RESTORE,
            DELTA_HEADER_RESTORE_SIZE,
            len(target_core),
            compare_size,
            len(records),
            block_size,
