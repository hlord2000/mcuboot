...
@click.option('--delta-revertible', is_flag=True, default=False,
              help='Include old bytes in the signed delta image so a test '
                   'update can be reverted from the same secondary-slot image.')
...
def sign(key, public_key_format, align, version, pad_sig, header_size,
    ...
    if delta_base is not None:
        ...
        print(f"delta reversible: {'yes' if delta_payload.reversible else 'no'}")
