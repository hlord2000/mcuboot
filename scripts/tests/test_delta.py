...
def sign_image(in_file: Path, out_file: Path, version: str, *extra_args: str):
    runner = CliRunner()
...
def apply_delta(base_signed: Path, patch_signed: Path, reverse: bool = False) -> bytes:
    ...
    magic, version, delta_header_size, target_size, write_size, record_count, block_size = \
        struct.unpack(delta.DELTA_HEADER, patch_payload[:delta.DELTA_HEADER_SIZE])
    flags = 0
    base_size = None

    if version == delta.DELTA_VERSION_RESTORE:
        magic, version, delta_header_size, target_size, write_size, record_count, \
            block_size, flags, base_size = struct.unpack(
                delta.DELTA_HEADER_RESTORE,
                patch_payload[:delta.DELTA_HEADER_RESTORE_SIZE])

    assert magic == delta.DELTA_MAGIC
    assert version in (delta.DELTA_VERSION, delta.DELTA_VERSION_RESTORE)
    ...
    for _ in range(record_count):
        ...
        off += rec_size
        if flags & delta.DELTA_FLAG_RESTORE:
            off += rec_size
...
def test_delta_image_reconstructs_signed_target(tmp_path: Path):
...
def test_revertible_delta_image_restores_signed_base(tmp_path: Path):
    base_raw = tmp_path / 'base.bin'
    target_raw = tmp_path / 'target.bin'
    base_signed = tmp_path / 'base-signed.bin'
    target_signed = tmp_path / 'target-signed.bin'
    patch_signed = tmp_path / 'target-delta-revertible.bin'
    ...
    assert 'delta reversible: yes' in result.output
    assert image_flags(patch_signed) & image.IMAGE_F['DELTA']
    assert patch_signed.stat().st_size < target_signed.stat().st_size
    assert apply_delta(base_signed, patch_signed) == \
        delta.image_core(target_signed.read_bytes(), 'little')
